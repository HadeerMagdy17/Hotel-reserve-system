...
// }
import { Modal, Box, Typography, Button, Stack } from "@mui/material";
import { useNavigate, useLocation } from "react-router-dom"; // 👈 استوردنا useLocation
...
export default function LoginModal({ open, onClose }: LoginModalProps) {
  const navigate = useNavigate();
  const location = useLocation(); // 👈 أخدنا مكان اليوزر الحالي
      ...
      <Box sx={{
        ...
        <Stack direction="row" spacing={2} sx={{ justifyContent: "center" }}>
          ...
          <Button 
            variant="contained" 
            onClick={() => {
              // 👈 بنبعت المسار الحالي (مثلاً /room-details/5) جوه الـ state تحت اسم from
